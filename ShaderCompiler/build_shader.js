...
const toolLocation = path.resolve(args[2] + "/bin/win32/sokol-shdc.exe");
if (fs.existsSync(toolLocation) == false) {
    console.error(`Shader compiler '${toolLocation}' does not exist!`);
    return;
}

// Beyond arg 2 are all the folders to process
const folders = args.slice(3);
console.log(`Shader folders: ${folders.join(", ")}`);
(async function () {
    for (let folder of folders) {
        ...
        try {
            ...
            for (let file of files) {
                ...
                const command = `"${toolLocation}" --input ${fullFile} --output ${genOutput} --slang glsl430:glsl300es:hlsl5:metal_macos:metal_ios:metal_sim:wgsl --ifdef`;
